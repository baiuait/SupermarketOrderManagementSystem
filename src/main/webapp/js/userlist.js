...
$(function(){
	...
	$(".viewUser").on("click",function(){
		...
		$.ajax({
			type : "GET",
			url : path+"/user/view", //多视图模式 使用json格式进行数据展示 即:application/json
			data : {id:obj.attr("userid"), /*format:"json" 多视图模式第二种写法*/},
			dataType : "json",
			success : function(result){
				if("nodata" == result){
					...
					$("#v_birthday").text(result.birthday);
					$("#v_phone").text(result.phone);
					$("#v_address").text(result.address);
					$("#v_userRoleName").text(result.userRoleName);
					$("#v_creation").text(result.creationDate);
				}
			},
			error : function(){
				alert("error!");
